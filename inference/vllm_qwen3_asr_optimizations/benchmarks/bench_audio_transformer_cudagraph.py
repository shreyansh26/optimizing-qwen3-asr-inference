#!/usr/bin/env python3
"""Gate the natural-audio row-bucket graph for the audio transformer on CUDA."""
    ...
    for thread_index, (outputs, expected_outputs) in enumerate(
        ...
        for iteration, (output, expected) in enumerate(
            ...
            if not torch.equal(output, expected):
                raise AssertionError(
                    "concurrent transformer replay is not bitwise exact for "
...
def _main() -> None:
    ...
    with torch.inference_mode():
        for case_index, segments in enumerate(cases):
            ...
            request_key = _make_transformer_graph_key(
                initial_hidden,
                cu_seqlens,
                max_seqlen,
                cu_values,
            ...
            eager_output = run_audio_transformer_eager(
                encoder,
                replay_hidden,
                cu_seqlens,
                max_seqlen,
            ...
            if not bitwise_exact:
                raise AssertionError(
                    f"transformer replay is not bitwise exact for {segments}"
                )
            ...
            bucket_eager_kernels = _cuda_kernel_names(
                lambda: run_audio_transformer_eager(
                    encoder,
            ...
            eager_call = lambda: run_audio_transformer_eager(  # noqa: E731
                encoder,
                replay_hidden,
                cu_seqlens,
                max_seqlen,
    ...
    print("gate=PASS_BUCKETED_AUDIO_TRANSFORMER_CUDAGRAPH")
