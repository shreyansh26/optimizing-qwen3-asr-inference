#!/usr/bin/env python3
"""CUDA gate for exact natural-hotset Qwen3-ASR audio-frontend graphs."""
...
from audio_cpu_metadata_pack_patch import run_audio_frontend_eager  # noqa: E402
from audio_frontend_cudagraph_patch import (
    ...
    run_audio_frontend_with_device_metadata,
)  # noqa: E402
from vllm.utils.torch_utils import async_tensor_h2d  # noqa: E402


class FrontendOnlyEncoder(nn.Module):
    ...
    *,
    ...
    for observation in range(1, _PROBATION_OBSERVATIONS + 1):
        ...
        if cache.entry_count != expected_entries:
            raise AssertionError(
                "frontend probation changed graph occupancy before observation 8"
            )
        if cache.admitted_key_count != expected_admitted:
            raise AssertionError(
                "frontend probation admitted a full key before observation 8"
            )
    ...
    for thread_index, (outputs, expected_outputs) in enumerate(
        ...
        for iteration, (output, expected) in enumerate(
            ...
            if not torch.equal(output, expected):
                raise AssertionError(
                    "concurrent frontend replay mismatch for "
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for the frontend graph gate")
    if min(
        args.warmup,
        args.repeats,
        args.replay_checks,
    ...
    encoder = FrontendOnlyEncoder().to(device=device).eval()
    cache = ExactShapeAudioFrontendGraphCache(
    ...
    with torch.inference_mode():
        for case_index, rows in enumerate(cases):
            ...
            alias_expected = run_audio_frontend_eager(
                encoder,
                alias_padded,
                *alias_metadata,
                async_tensor_h2d=async_tensor_h2d,
            ...
            if not bitwise_exact:
                raise AssertionError(f"frontend replay mismatch for rows={rows}")
            ...
            graph_body = lambda: run_audio_frontend_with_device_metadata(  # noqa: E731
                encoder,
                primary_padded,
            ...
            if not kernel_order_exact:
                raise AssertionError(
                    "captured frontend kernel names/order differ from eager "
                    f"for rows={rows}: eager={eager_kernels}, graph={graph_kernels}"
                )

            eager_call = lambda: run_audio_frontend_eager(  # noqa: E731
                encoder,
                primary_padded,
                *primary_metadata,
                async_tensor_h2d=async_tensor_h2d,
    ...
    print("gate=PASS_EXACT_NATURAL_AUDIO_FRONTEND_CUDAGRAPH")
